import type { ConverterContext, ConverterResult, DTCGTypographyValue, SubToken } from '../lib.js';
...
export function convertTypography(value: unknown, context: ConverterContext): ConverterResult {
  const typography = value as DTCGTypographyValue;

  if (!typography || typeof typography !== 'object') {
    context.logger.warn({
      group: 'plugin',
      label: PLUGIN_NAME,
      message: `Token "${context.tokenId}" has invalid typography value: expected object, got ${typeof typography}`,
    });
  ...
  if (typography.lineHeight !== undefined) {
    const aliasOf = getSubTokenAlias(partialAliasOf?.lineHeight, 'lineHeight', context.allTokens);
