...
function roundAndClamp(value: number): number {
  const rounded =
    Math.round(value * 10 ** COLOR_PRECISION) / 10 ** COLOR_PRECISION
...
export function convertColor(
  ...
  if (
    FIGMA_COLOR_SPACES.includes(
      color.colorSpace as (typeof FIGMA_COLOR_SPACES)[number],
    )
  ) {
    // Handle 'none' values in components
    const components = color.components.map((c) => (c === "none" ? 0 : c)) as [
      number,
      number,
      number,
    ]
  ...
  try {
    ...
    const srgbChannels = normalizeComponents(
      srgbColor.coords as [number, number, number],
    )
